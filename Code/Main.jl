...
lb = 1e-006 * ones(size(δ_guess))
ub = 10 * ones(size(δ_guess))
ub[6] = 6.00
...
function objectivefunction(δ_guess, σ, θ, κ, distrw, comlang, corrup, N, ξ,
    S, prod_draw_uniform, weights_prod, fc_shock_randn, num_rand_checks,
    ...
    fc_mean = δ_guess[2] .* ((distrw').^δ_guess[3]) .* δ_guess[4].^(comlang') .* exp.(-δ_guess[5] .* corrup')
    temp = fc_shock_randn.*δ_guess[6]
    ...
    for firm in 1:S
        ...
        Z = optimalset(Z, gap_bounds, firm, Z_lb, Z_ub, S, N, num_rand_checks,
            rand_check_matrix, fc, ξ, my_exp, ϕ_σ_B)
    end
